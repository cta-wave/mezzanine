...
def genSimpleFrameImages(widthHeightPixelsTuple, flashColourGen, numFrames, BG_COLOUR=(0,0,0), frameSkipChecker=None):
    ...
    for frameNum in range(0,numFrames):
        ...
        # create black image and an object to let us draw on it
        img = Image.new("RGB", (widthPixels, heightPixels), color=BG_COLOUR)
        draw = ImageDraw.Draw(img)

        # draw a flashing rectangular box that fills the entire image
        flashColour = flashCols[frameNum]
 
        topLeft     = [0,0]
        bottomRight = [widthPixels-1,heightPixels-1]
        draw.rectangle(topLeft + bottomRight, outline=None, fill=flashColour)
